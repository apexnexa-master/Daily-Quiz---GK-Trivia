...
// Automatic daily quiz generation and management with timing control
...
class QuizSchedulerService {
  ...
  // Default quiz timing (can be configured by admin)
  ...
  Future<QuizModel?> prepareDailyQuiz(String examMode) async {
    ...
    try {
      ...
      // Firestore failed, continue to local
    }

    // 2. Try local questions
  ...
  Future<QuizModel?> _createDailyQuizFromLocal(
      String examMode, String date) async {
    try {
      // Get questions from local data directly
  ...
  Future<QuizModel?> _createDailyQuizFromPractice(
      String examMode, String date) async {
    try {
  ...
  Future<void> _saveDailyQuiz(
      String examMode, String date, List<QuestionModel> questions) async {
  ...
  Future<Set<String>> _getRecentlySeenQuestionIds(int count) async {
    ...
    try {
      ...
      for (int i = 0; i < count; i++) {
        ...
        final quizDoc =
            await _db.collection('quizzes').doc('${dateStr}_GENERAL').get();
        if (quizDoc.exists) {
          final questionsSnapshot = await _db
              .collection('quizzes')
