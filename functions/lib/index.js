...
exports.generateChallenge = exports.submitChallengeScore = exports.updateStreak = exports.updateLeaderboard = exports.submitAttempt = exports.dailyQuizGenerator = void 0;
...
// ══════════════════════════════════════════════════════════════
exports.dailyQuizGenerator = functions
...
async function createQuizForMode(date, examMode) {
    ...
    questions.forEach((q, i) => {
        ...
        qBatch.set(ref, Object.assign(Object.assign({}, q), { order: i + 1 }));
    });
    await qBatch.commit();
}
// Mock question generator — replace with real CMS/AI in production
function generateMockQuestions(date, _examMode) {
...
async function sendDailyNotification(date) {
    const message = {
        notification: {
...
exports.submitAttempt = functions
    ...
    const { quizId, answers, timeTaken } = data;
    ...
    questionsSnap.docs.forEach((doc, i) => {
        if (answers[i] === doc.data().correct_index)
            score++;
    });
...
// ══════════════════════════════════════════════════════════════
async function updateLeaderboardEntry(uid, quizDate, weekId, examMode, score, timeTaken) {
...
exports.updateLeaderboard = functions
    ...
    await updateLeaderboardEntry(uid, data.quiz_date, data.week_id, data.exam_mode, data.score, data.time_taken);
});
// ══════════════════════════════════════════════════════════════
// 4. STREAK UPDATER
//    Logic:
...
//    - Older → reset to 1
// ══════════════════════════════════════════════════════════════
async function updateStreakInternal(uid, quizDate) {
...
exports.updateStreak = functions
    .region('asia-south1')
    .https
    .onCall(async (data, context) => {
    if (!context.auth)
...
async function updateUserStats(uid, score) {
...
exports.submitChallengeScore = functions
    .region('asia-south1')
    .https
    .onCall(async (data, context) => {
    if (!context.auth) {
    ...
    const challengeId = data === null || data === void 0 ? void 0 : data.challengeId;
    const timeTaken = data === null || data === void 0 ? void 0 : data.timeTaken;
    if (typeof score !== 'number' || !Number.isInteger(score) ||
        score < 0 || score > 1000) {
        throw new functions.https.HttpsError('invalid-argument', 'score must be an integer in 0..1000');
    }
    if (typeof challengeId !== 'string' || !CHALLENGE_ID_PATTERN.test(challengeId)) {
        throw new functions.https.HttpsError('invalid-argument', 'challengeId must be a valid daily challenge id');
    }
    const t = (typeof timeTaken === 'number' && timeTaken >= 0) ? Math.floor(timeTaken) : 0;
    const today = getISTDate();
    if (!challengeId.startsWith(`DC-${today}-`)) {
        throw new functions.https.HttpsError('invalid-argument', 'challenge is not valid for today');
    }
    const uid = context.auth.uid;
...
async function getDisplayName(uid) {
    ...
    return ((_a = snap.data()) === null || _a === void 0 ? void 0 : _a.display_name) || 'Guest';
}
// ══════════════════════════════════════════════════════════════
...
exports.generateChallenge = functions
    ...
    const uid = context.auth.uid;
    const userSnap = await db.collection('users').doc(uid).get();
    const userName = ((_a = userSnap.data()) === null || _a === void 0 ? void 0 : _a.display_name) || 'A friend';
    const challengeId = (0, uuid_1.v4)();
    const domain = ((_b = functions.config().app) === null || _b === void 0 ? void 0 : _b.deep_link_domain) || 'gkquiz.yourapp.com';
